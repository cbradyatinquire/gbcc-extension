...
;;Sharing and Storing Procedures

to share
  if any? turtles [
    if not ( user-yes-or-no? "Are you sure you want to share when your experiement isn't complete?" )
    [
      stop
    ]
  ]
  ...
  gbcc:broadcast-text (word num-particles " particles, wiggle angle of " wiggle-angle )
end
...
to gbcc-on-enter [ user-id role ]
  ...
  show "TODO:  when this event happens, do nothing, I think."
end
...
to gbcc-on-select [ user-id role ]
  ...
  ;;show "TODO:  when this event happens, replace all the globals of this model from those of the selected user."
...
to gbcc-on-deselect [ user-id role ]
  ...
  show "TODO:  when this event happens, resort to my stored globals (this is one option; the other is to do nothing)."
end
